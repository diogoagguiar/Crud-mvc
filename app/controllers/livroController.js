...
exports.create = [
    body('ISBN').isLength({ min: 10, max: 13 }).withMessage('ISBN deve ter entre 10 e 13 caracteres')
        .matches(/^(?:ISBN(?:-13)?:?)(?=[0-9]{13}$)([0-9]{3}-){2}[0-9]{3}[0-9X]$|^[0-9]{10}$|^[0-9]{13}$/).withMessage('ISBN inválido'),
    ...
    body('ano_publicacao').isInt({ min: 1000, max: new Date().getFullYear() }).withMessage('Ano de publicação inválido'),
    async (req, res) => {
        ...
        try {
            ...
            // Lidar com erro de ISBN duplicado, por exemplo
            let errorMessage = 'Erro ao criar livro.';
            if (error.code === 'ER_DUP_ENTRY') {
                errorMessage = 'O ISBN fornecido já existe.';
            }
            return res.render('livros/create', { errors: [{ msg: errorMessage }], old: req.body });
        }
    }
];
...
exports.editForm = async (req, res) => {
    ...
    if (!livro) {
        return res.redirect('/livros'); // Ou renderizar uma página de erro
    }
